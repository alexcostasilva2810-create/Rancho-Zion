...
# =================================================================
# BLOCO 7: TELA DE DECLARAÇÃO (CORREÇÃO RICH_TEXT NOTION)
# =================================================================
    ...
    with st.form("form_declaracao_final"):
        ...
        necessidades_extras = st.text_area("Considerações:", value="...")
        
        st.write("Assinatura Digital:")
        canvas_result = st_canvas(fill_color="rgba(255, 255, 255, 0)", stroke_width=3, stroke_color="#000000",
                                background_color="#FFFFFF", height=120, drawing_mode="freedraw", key="ass_final_v3")
        enviar = st.form_submit_button("💾 SALVAR E GERAR PDF OFICIAL")

    if enviar:
        if canvas_result.image_data is not None:
            try:
                # --- 1. SALVAMENTO NO NOTION (ESTRUTURA CORRETA) ---
                payload = {
                    ...
                    "properties": {
                        ...
                        # FORMATO RICH_TEXT PARA AS COLUNAS DE TEXTO
                ...
                if res.status_code == 200 or res.status_code == 201:
                    ...
                    st.error(f"Erro {res.status_code} no Notion. Verifique se os nomes das colunas estão IGUAIS (maiúsculas/minúsculas).")

                # --- 2. GERAÇÃO DO PDF (MANTENDO O SEU PADRÃO) ---
                class PDF_Final(FPDF):
                    def footer(self):
                        self.set_y(-15); self.set_font('Arial', 'I', 8)
                        agora = datetime.now() - timedelta(hours=3)
                        self.cell(0, 10, f'Gerado em: {agora.strftime("%d/%m/%Y %H:%M:%S")}', 0, 0, 'C')

                pdf = PDF_Final(); pdf.add_page()
                ...
                pdf.cell(0, 10, prep("DECLARAÇÃO DE REABASTECIMENTO"), ln=True, align="C")
                pdf.ln(10)
                pdf.set_font("Arial", "", 12); pdf.multi_cell(0, 8, prep(f"Certifico que a lotação é de {lotacao} tripulantes."), align="J")
                
                img_ass = Image.fromarray(canvas_result.image_data.astype('uint8'), 'RGBA')
                img_ass.save("temp_ass.png")
                pdf.ln(15); pdf.cell(0, 10, prep("__________________________________________"), ln=True, align="C")
                pdf.image("temp_ass.png", x=75, y=pdf.get_y()-17, w=60)
                
                pdf_output = pdf.output(dest='S').encode('latin-1')
                st.download_button("📥 BAIXAR PDF OFICIAL", data=pdf_output, file_name=f"Declaracao_{st.session_state.navio}.pdf", mime="application/pdf", use_container_width=True)
