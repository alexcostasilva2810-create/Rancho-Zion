...
# --- BLOCO 6: CONFERÊNCIA DE ESTOQUE (RESTAURADO) ---
    ...
    df_editado = st.data_editor(st.session_state.df_lista, column_config={"ITEM": st.column_config.NumberColumn("CÓD.", disabled=True), "CONFIRMA": st.column_config.NumberColumn("SUA QTD", min_value=0)}, hide_index=True, use_container_width=True, key="editor_estoque_vFinal")
    ...
    with col_pdf:
        if st.button("💾 GERAR E SALVAR RELATÓRIO"):
            try:
                ...
                st.download_button(label="📥 BAIXAR PDF DO ESTOQUE", data=pdf_bytes, file_name=f"Rancho_{st.session_state.navio}.pdf", mime="application/pdf", use_container_width=True)
